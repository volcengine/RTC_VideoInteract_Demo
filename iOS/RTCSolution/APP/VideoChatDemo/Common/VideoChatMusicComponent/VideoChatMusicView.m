...
@implementation VideoChatMusicView
...
- (void)switchViewChanged:(UISwitch *)sender {
    if (sender.on) {
        if (_musicIsOpen) {
            [[VideoChatRTCManager shareRtc] resumeBackgroundMusic];
        } else {
            NSString *path = [[NSBundle mainBundle] pathForResource:@"videochat_bg"
                                                             ofType:@"mp3"];
            _musicIsOpen = YES;
            [[VideoChatRTCManager shareRtc] startBackgroundMusic:path];
        }
...
- (void)musicSliderValueChanged:(UISlider *)musicSlider {
    [[VideoChatRTCManager shareRtc] setMusicVolume:musicSlider.value];
...
- (void)vocalSliderValueChanged:(UISlider *)musicSlider {
    [[VideoChatRTCManager shareRtc] setRecordingVolume:musicSlider.value];
...
- (UILabel *)messageLabel {
    if (!_messageLabel) {
        ...
        _messageLabel.text = LocalizedString(@"volume");
