//
//  VideoChatPKUserListComponents.m
//  veRTC_Demo
//
...
- (void)showAnchorList {
    if (self.isPKWaitingReply) {
        [[ToastComponents shareToastComponents] showWithMessage:@"当前处于邀请状态"];
        return;
    }
...
- (void)videoChatPKUserListView:(VideoChatPKUserListView *)view didClickUserModel:(VideoChatUserModel *)userModel {
    ...
    [VideoChatRTMManager requestInviteAnchorWithFromRoomID:self.roomModel.roomID
        ...
        if (model.result) {
            NSString *message = [NSString stringWithFormat:@"已向%@发出邀请，等待对方应答", userModel.name];
...
- (void)receivedAnchorPKInvite:(VideoChatUserModel *)anchorModel {
    ...
    [[AlertActionManager shareAlertActionManager] showWithMessage:[NSString stringWithFormat:@"%@邀请你进行主播连线，是否接受？", anchorModel.name] actions:@[cancelModel, alertModel]];
