...
func (rs *RoomService) CreateRoom(ctx context.Context, appID, roomName, roomBackgroundImageName, hostUserID, hostUserName, hostDeviceID string) (*Room, *User, error) {
	...
	host := rs.userFactory.NewUser(ctx, appID, room.GetRoomID(), hostUserID, hostUserName, hostDeviceID, vi_db.UserRoleHost)
...
func (rs *RoomService) FinishLive(ctx context.Context, appID, roomID string, finishType int) error {
	...
	if room.IsAnchorInteract() {
		...
		otherRoomID, err := interactService.FinishAnchor(ctx, appID, roomID, room.GetHostUserID())
		if err != nil {
			logs.CtxError(ctx, "invite failed,error:%s", err)
		}
		room.Status = vi_db.RoomStatusStart
		rs.roomFactory.Save(ctx, room)

		otherRoom, err := rs.roomFactory.GetRoomByRoomID(ctx, appID, otherRoomID)
		if otherRoom != nil {
			otherRoom.Status = vi_db.RoomStatusStart
			rs.roomFactory.Save(ctx, otherRoom)
		}
	}
	...
	err = rs.userFactory.UpdateUsersByRoomID(ctx, appID, roomID, map[string]interface{}{
		"net_status":      vi_db.UserNetStatusOffline,
...
func (rs *RoomService) JoinRoom(ctx context.Context, appID, roomID, userID, userName, userDeviceID string) error {
	...
	user := rs.userFactory.NewUser(ctx, appID, roomID, userID, userName, userDeviceID, vi_db.UserRoleAudience)
...
func (rs *RoomService) Disconnect(ctx context.Context, appID, roomID, userID string) {
	...
	logs.CtxInfo(ctx, "vi disconnect user:%#v", user.ViUser)
