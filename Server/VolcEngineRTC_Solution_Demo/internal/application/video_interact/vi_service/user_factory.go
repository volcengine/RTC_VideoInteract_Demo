...
func (uf *UserFactory) NewUser(ctx context.Context, appID, roomID, userID, userName, deviceID string, userRole int) *User {
	dbUser := &vi_entity.ViUser{
		AppID:          appID,
		RoomID:         roomID,
		UserID:         userID,
		UserName:       userName,
		UserRole:       userRole,
		NetStatus:      vi_db.UserNetStatusOffline,
		InteractStatus: vi_db.UserInteractStatusNormal,
		SeatID:         0,
		Mic:            1,
		Camera:         1,
		CreateTime:     time.Now(),
...
func (uf *UserFactory) Save(ctx context.Context, user *User) error {
	if user.IsDirty() {
		...
		err := uf.userRepo.Save(ctx, user.ViUser)
		if err != nil {
...
func (uf *UserFactory) GetActiveAnchorList(ctx context.Context, appID string) ([]*User, error) {
	dbAudiences, err := uf.userRepo.GetActiveAnchors(ctx, appID)
	if err != nil {
		return nil, custom_error.InternalError(err)
	}
