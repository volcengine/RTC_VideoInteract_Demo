...
type closeChatRoomReq struct {
	RoomID     string `json:"room_id"`
	UserID     string `json:"user_id"`
	LoginToken string `json:"login_token"`
}

type closeChatRoomResp struct {
}

func (eh *EventHandler) CloseChatRoom(ctx context.Context, param *public.EventParam) (resp interface{}, err error) {
	logs.CtxInfo(ctx, "viAgreeApply param:%+v", param)
	var p closeChatRoomReq
	...
	interactService.ClearRoom(ctx, param.AppID, room.GetRoomID())

	informer := inform.GetInformService(param.AppID)

	data := &vi_service.InformCloseChatRoom{
		RoomID: room.GetRoomID(),
	}
	informer.BroadcastRoom(ctx, room.GetRoomID(), vi_service.OnCloseChatRoom, data)
