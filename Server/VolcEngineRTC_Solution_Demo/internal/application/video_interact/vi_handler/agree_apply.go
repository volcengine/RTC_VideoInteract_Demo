...
func (eh *EventHandler) AgreeApply(ctx context.Context, param *public.EventParam) (resp interface{}, err error) {
	...
	err = interactService.HostReply(ctx, param.AppID, p.RoomID, p.UserID, p.AudienceUserID, vi_service.InteractReplyTypeAccept)
	if err != nil {
		logs.CtxError(ctx, "invite failed,error:%s", err)
		return nil, err
	}

	room.Status = vi_db.RoomStatusAudience
