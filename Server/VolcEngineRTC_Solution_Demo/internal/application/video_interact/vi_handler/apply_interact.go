...
func (eh *EventHandler) ApplyInteract(ctx context.Context, param *public.EventParam) (resp interface{}, err error) {
	logs.CtxInfo(ctx, "viApplyInteract param:%+v", param)
	...
	data := &vi_service.InformApplyInteract{
		UserInfo: user,
		SeatID:   p.SeatID,
	}
	informer.UnicastUser(ctx, room.GetRoomID(), room.GetHostUserID(), vi_service.OnApplyInteract, data)
